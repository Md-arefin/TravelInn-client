...
const Details = ({ hotel, CSS }) => {
    ...
    return (
        <div key={_id} className={`card bg-base-100 ${CSS == 'flex' ? 'h-[950px] ' : "h-[620px]"}   shadow-xl relative`}>
            <img className={`w-full object-cover rounded-t-2xl  ${CSS == 'flex' ? 'h-[500px] ' : "h-[280px]"}`} src={images} alt={name} />
